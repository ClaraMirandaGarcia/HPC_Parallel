...
{
    PGM_IMG result;
    int hist[256], hist_comp[256];

    result.w = img_in.w;
    result.h = img_in.h;
    result.init_offst = img_in.init_offst;
    result.size_sect = img_in.size_sect;
    result.img = (unsigned char *)malloc(result.size_sect*sizeof(unsigned char));
    
    histogram(hist, img_in.img, img_in.size_sect, 256);

    MPI_Allreduce(hist, hist_comp, 256, MPI_INT, MPI_SUM, MPI_COMM_WORLD);

    histogram_equalization(result.img,img_in.img,hist_comp,result.w*result.h, 256, result.size_sect);
    return result;
}
...
PPM_IMG contrast_enhancement_c_rgb(PPM_IMG img_in)
{
    ...
    histogram_equalization(result.img_r,img_in.img_r,hist,result.w*result.h, 256, result.w*result.h);
    histogram(hist, img_in.img_g, img_in.h * img_in.w, 256);
    histogram_equalization(result.img_g,img_in.img_g,hist,result.w*result.h, 256, result.w*result.h);
    histogram(hist, img_in.img_b, img_in.h * img_in.w, 256);
    histogram_equalization(result.img_b,img_in.img_b,hist,result.w*result.h, 256, result.w*result.h);
...
PPM_IMG contrast_enhancement_c_yuv(PPM_IMG img_in)
{
    ...
    y_equ = (unsigned char *)malloc(yuv_med.size_sect*sizeof(unsigned char));
    
    histogram(hist, yuv_med.img_y, yuv_med.size_sect, 256);

    MPI_Allreduce(hist, hist_comp, 256, MPI_INT, MPI_SUM, MPI_COMM_WORLD);

    histogram_equalization(y_equ, yuv_med.img_y, hist_comp, yuv_med.h * yuv_med.w, 256, yuv_med.size_sect);
...
PPM_IMG contrast_enhancement_c_hsl(PPM_IMG img_in)
{
    ...
    l_equ = (unsigned char *)malloc(hsl_med.size_sect*sizeof(unsigned char));

    histogram(hist, hsl_med.l, hsl_med.size_sect, 256);

    MPI_Allreduce(hist, hist_comp, 256, MPI_INT, MPI_SUM, MPI_COMM_WORLD);

    histogram_equalization(l_equ, hsl_med.l, hist_comp, hsl_med.width*hsl_med.height, 256, hsl_med.size_sect);
...
HSL_IMG rgb2hsl(PPM_IMG img_in)
{
    ...
    img_out.init_offst = img_in.init_offst;
    img_out.size_sect = img_in.size_sect;
    img_out.h = (float *)malloc(img_in.size_sect * sizeof(float));
    img_out.s = (float *)malloc(img_in.size_sect * sizeof(float));
    img_out.l = (unsigned char *)malloc(img_in.size_sect*sizeof(unsigned char));
    
    #pragma omp parallell for schedule(guided)
    for(i = 0; i < img_in.size_sect; i ++){
...
YUV_IMG rgb2yuv(PPM_IMG img_in)
{
    ...
    img_out.init_offst = img_in.init_offst;
    img_out.size_sect = img_in.size_sect;
    img_out.img_y = (unsigned char *)malloc(img_out.size_sect*sizeof(unsigned char));
    img_out.img_u = (unsigned char *)malloc(img_out.size_sect*sizeof(unsigned char));
    img_out.img_v = (unsigned char *)malloc(img_out.size_sect*sizeof(unsigned char));

    #pragma omp parallell for schedule(guided)
    for(i = 0; i < img_out.size_sect; i ++){
        r = img_in.img_r[i];
        g = img_in.img_g[i];
        b = img_in.img_b[i];
...
PPM_IMG yuv2rgb(YUV_IMG img_in)
{
    ...
    img_out.init_offst = img_in.init_offst;
    img_out.size_sect = img_in.size_sect;
    img_out.img_r = (unsigned char *)malloc(img_out.size_sect*sizeof(unsigned char));
    img_out.img_g = (unsigned char *)malloc(img_out.size_sect*sizeof(unsigned char));
    img_out.img_b = (unsigned char *)malloc(img_out.size_sect*sizeof(unsigned char));
